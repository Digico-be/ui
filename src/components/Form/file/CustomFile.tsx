...
export const CustomFile = ({ name }: Props) => {
    ...
    const onDrop = useCallback(
        async (acceptedFiles: File[]) => {
            try {
                ...
                const fileConverted = new File([compressedFile], acceptedFile.name, { type: compressedFile.type, lastModified: acceptedFile.lastModified })

                setValue(`${name}.url`, URL.createObjectURL(fileConverted))
                setValue(`${name}.file`, fileConverted)
            } catch {
                ...
                <div className="h-full relative group cursor-pointer bg-grey-200 rounded border border-grey-400 justify-center hover:border-grey-600 flex flex-col gap-4 items-center">
                    <div className="h-full w-full overflow-x-auto p-4 flex gap-4 items-center">
                        <div className="h-full relative flex-shrink-0">
                            ...
                            <img className="rounded w-full h-full object-cover object-center" src={watch(`${name}.url`)} alt={`Preview`} />
                        </div>
                    </div>
                </div>
            ) : (
